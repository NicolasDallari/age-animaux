<!DOCTYPE html>
<html lang="fr">

<head>
    <!-- Définition de l'encodage et de la vue mobile -->
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    
    <!-- Métadonnées pour le référencement -->
    <meta name="description" content="Calculez facilement l'âge de votre animal préféré avec notre outil en ligne.">
    <meta name="keywords" content="âge animal, calcul âge, chat, chien, animal de compagnie">
    <meta name="author" content="Dallari Nicolas">
    <meta name="robots" content="index, follow">
    
    <!-- Canonical pour éviter le contenu dupliqué -->
    <link rel="canonical" href="https://www.age-animaux.dallarinicolas.fr/">
    
    <!-- Préchargement des polices pour optimiser le rendu -->
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Lavishly+Yours&family=Playfair+Display:ital,wght@0,400..900;1,400..900&display=swap" rel="stylesheet">
    
    <!-- Feuille de style principale -->
    <link rel="stylesheet" href="assets/css/style.css">
    
    <!-- Bibliothèque GSAP pour les animations -->
    <script src="https://cdn.jsdelivr.net/npm/gsap@3.12.7/dist/gsap.min.js"></script>
    
    <!-- Favicon -->
    <link rel="icon" href="assets/images/home/animaux-domestiques.png" type="image/png" sizes="32x32">
</head>

<body>

<nav class="mobile">
    <a  href="/">
        <img class="logoMaison" src="assets/images/home/maison.webp" alt="logo maison"  >
    </a>
    <a href="/chien">
        <img class="logochien" src="assets/images/home/logochien.png" alt="logo chien">
    </a>
    <a href="/chat">
        <img class="logochat" src="assets/images/home/chat.webp" alt="logo chat">
    </a>
</nav>
    
